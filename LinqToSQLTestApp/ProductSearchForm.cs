        ...
        private void ProductSearchForm_Activated(object sender, EventArgs e)
        {
            using(var dc = new DemoDatabaseDataContext())
            {
                ...
                if(cboCategoryName.Items.Count == 0)
                {
                    ...
                    cboCategoryName.Items.AddRange(dc.spCategories().Select(c => c.CategoryName).ToArray());

                    cboCompanyName.Items.Add(String.Empty);
                    cboCompanyName.Items.AddRange(dc.spCompanies().Select(c => c.CompanyName).ToArray());
